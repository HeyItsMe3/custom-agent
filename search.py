...
class WebSearcher:
    ...
    def generate_searches(self, plan, query):
        ...
        search_queries = function_args["search_engine_queries"]
    ...
    def get_search_page(self, search_results, plan, query):
        ...
        tool_calls = chat_response.choices[0].message.tool_calls[0]
        function_args = json.loads(tool_calls.function.arguments)
        search_queries = function_args["best_search_page"]
    ...
    def scrape_website_content(self, website_url):
        ...
        try:
            ...
            web_content = f"Source: {website_url}, content: {clean_text}"
            return web_content # {website_url: clean_text}
    ...
    def use_tool(self, plan=None, query=None):
        ...
        results_lists = search.scrape_website_content(best_page)

        #add a similarity search section here => this wont work in case someones trying to find top news
        # why not have a seperate func to check the token size of result if it is more then summarize it

        model_name = "sentence-transformers/all-MiniLM-L12-v2"
        model_embedding = HuggingFaceEmbeddings(model_name=model_name, model_kwargs = {'device': 'cuda'})
        text_splitter = RecursiveCharacterTextSplitter(chunk_size = 512, chunk_overlap = 20)
        texts = text_splitter.create_documents([results_lists])
        #model_embedding = SentenceTransformer('all-MiniLM-L12-v2', device='cuda')
        db = Chroma.from_documents(texts, model_embedding)
        retriever = db.as_retriever(search_kwargs={"k":3})
        #input_em = embedding_model.encode(query).tolist()
        results_final = retriever.invoke(query)


        if self.verbose:
            print(colored(f"SEARCH RESULTS {search_results}", 'yellow'))
            print(colored(f"RESULTS DICT {results_final}", "red"))
